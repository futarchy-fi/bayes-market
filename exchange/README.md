...
report-only unless both `--execute` and `--enable-live-trading` are supplied.
...
After inspecting a report-only pass, set `FUTARCHY_API_KEY` and run one live
canary pass:

```bash
python -m exchange.agents.arb --once --execute --enable-live-trading
```

For continuous execution after inspecting the canary:

```bash
python -m exchange.agents.arb --execute --enable-live-trading --interval 30
```
